...
class Mode extends DatabaseRow
{
	...
	{
		$direction_from = strtolower($direction_from);
		$direction_to = strtolower($direction_to);
		
		$result = Connection::query("SELECT * FROM modes WHERE LOWER(`from`) = '$direction_from' ".
									"AND LOWER(`to`) = '$direction_to'");
		if (!!($error = Connection::query_error_clear()))
		{
			return static::errors_push("Failed to select from mode: " . $error . ".");
		}
	...
	{
		return static::errors_push("Mode deletion forbidden.");
	}
